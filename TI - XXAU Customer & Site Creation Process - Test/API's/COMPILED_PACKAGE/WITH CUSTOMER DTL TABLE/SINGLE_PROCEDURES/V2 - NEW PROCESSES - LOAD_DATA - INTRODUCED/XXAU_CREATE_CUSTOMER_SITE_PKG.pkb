...
IS
			...
			IF P_PAYMENT_TERMS IS NOT NULL THEN
				BEGIN
					...
					FROM
						APPS.RA_TERMS
					WHERE
						UPPER(NAME) = UPPER(TRIM(P_PAYMENT_TERMS)); --CREATE_SITE_REC.PAYMENT_TERMS;
			...
			EXCEPTION
				WHEN OTHERS THEN
					BEGIN
						...
						LB_STATUS := 	FND_FLEX_KEYVAL.VALIDATE_SEGS(  OPERATION 		=> 'CREATE_COMBINATION'
	 ...
	 * AKALA            1.2         09-APR-2026     Updated the logic according to XXAU_CUSTOMER_DETAILS_TEST TABLE
     ***************************************************************************************************/	
	...
	SELECT DISTINCT 
			...
			, B.UNIT 
			, B.CUSTOMER_TYPE
			, B.CUSTOMER_CLASS                          CUSTOMER_CLASS_CODE
			, B.MOBILE_NO
			, B.CONTACT_PERSON
			, NVL(B.ACCOUNT_DESCRIPTION, B.CUSTOMER_NAME) ACCOUNT_DESCRIPTION
			, B.ABC_TYPE
			, B.OLD_PARTY_ID
			, B.OLD_CUSTOMER_ID
			, B.AGENCY_ID
	...
	, XXAU_CUSTOMER_DETAILS_TEST B
	WHERE A.AGENCY_ID  IN (SELECT AGENCY_ID 
	...
	CURSOR C2(P_AGENCY_ID NUMBER
		...
		SELECT 
			...
			, B.UNIT
			, B.CUSTOMER_TYPE
			, B.CUSTOMER_CLASS CUSTOMER_CALSS_CODE
			, B.ADDRESS1
			, B.ADDRESS2
			, B.ADDRESS3
			, B.ADDRESS4
			, B.CITY
			, B.STATE
			, DECODE( UPPER(B.COUNTRY), 'INDIA','IN',UPPER(B.COUNTRY)) COUNTRY
			, B.COUNTY
			, B.LOCATION1
			, B.STATION_NAME
			, A.NAME_OF_CENTRE
			, B.PINCODE        POSTAL_CODE
			, 'SHIP_TO'      SITE_USE_CODE
			, DECODE(UPPER(B.PRIMARY_SITE_FALG),'YES','Y','N')              PRIMARY_FLAG
			, 'A'  STATUS
			, B.REC_ACCOUNT
			, B.PAYMENT_TERMS
			, A.ERP_PROCESS_FLAG			
	...
	BEGIN
		...
		LOOP
			...
			BEGIN
				CREATE_CUST_ACCOUNT (  P_ORGANIZATION_NAME 		=> I.CUSTOMER_NAME||'- New Safalta Unit'
				...
				THEN
						...
						-- Resetting the Variables For Correct Values And Updation
							...
							EXCEPTION
								WHEN OTHERS THEN
									CREATE_LOCATION (  P_ADDRESS1 		=> J.ADDRESS1
							...
							THEN
								BEGIN
									...
									WHERE 1=1
										...
										AND PARTY_SITE_NAME = J.STATION_NAME;
								EXCEPTION
									WHEN OTHERS THEN
							...
							THEN
								...
								CREATE_CONTACT_ORG ( P_PARTY_ID 		=> LN_PARTY_ID
								...
								--Start:  Added by AKALA on 10-APR-2026, to create or get CCID before creating the SITE USES
								IF LN_CCID_S < 0
								THEN
									BEGIN
										SELECT CODE_COMBINATION_ID
										...
										WHERE CONCATENATED_SEGMENTS = J.REC_ACCOUNT
										;
									EXCEPTION 
										WHEN OTHERS THEN 
						...
						FND_FILE.PUT_LINE(FND_FILE.LOG,RPAD('*',25,'*'));
				ELSE 
					FND_FILE.PUT_LINE(FND_FILE.LOG,'LN_CUST_ACCOUNT_ID: '||LN_CUST_ACCOUNT_ID||'; PARTY_ID: '||LN_PARTY_ID||'; Msg : '||LC_MSG);
			...
			*/
			...
			--COMMIT;
				FND_FILE.PUT_LINE(FND_FILE.LOG,'LN_CUST_ACCOUNT_ID: '||LN_CUST_ACCOUNT_ID||'; PARTY_ID: '||LN_PARTY_ID||'; Msg : '||LC_MSG);
				FND_FILE.PUT_LINE(FND_FILE.LOG,'LN_LOCATION_ID: '||LN_LOCATION_ID||'; Msg : '||LC_MSG);
				FND_FILE.PUT_LINE(FND_FILE.LOG,'LN_PARTY_SITE_ID: '||LN_PARTY_SITE_ID||'; Msg : '||LC_MSG);
				FND_FILE.PUT_LINE(FND_FILE.LOG,'LN_CUST_ACCT_SITE_ID: '||LN_CUST_ACCT_SITE_ID||'; Msg : '||LC_MSG);
				FND_FILE.PUT_LINE(FND_FILE.LOG,'LN_SITE_USE_ID: '||LN_SITE_USE_ID||'; Msg : '||LC_MSG);
				FND_FILE.PUT_LINE(FND_FILE.LOG,RPAD('*',25,'*'));
     ...
     * AKALA            1.2         08-APR-2026     1. Upgraded code for EMAIL and PHONE creation in ERP
													2. Added logic for CREATE_CODE_COMBINATION
     * AKALA            1.3         09-APR-2026     Updated the logic according to XXAU_CUSTOMER_DETAILS_TEST TABLE
     ***************************************************************************************************/		
	...
	BEGIN
		...
		LOOP
			...
			FND_FILE.PUT_LINE(FND_FILE.LOG,RPAD('+',25,'+'));
			BEGIN
				CREATE_CUST_ACCOUNT (    P_ORGANIZATION_NAME 		=> I.CUSTOMER_NAME
										,  P_ACCOUNT_NAME 			=> I.ACCOUNT_DESCRIPTION
										,  P_ABC_TYPE 				=> I.ABC_TYPE
										,  P_AGENCY_ID 				=> I.AGENCY_ID
										,  P_CUSTOMER_TYPE 			=> I.CUSTOMER_TYPE
		...
		LOOP
			...
			BEGIN				
				...
				WHEN OTHERS THEN 
					...
					LN_PARTY_ID := -1;
					I.ERP_ERR_MSG := 'ERP Customer not available : '||I.ERP_CUSTOMER_NUMBER;
					I.ERP_PROCESS_FLAG := 'E';
				END;
