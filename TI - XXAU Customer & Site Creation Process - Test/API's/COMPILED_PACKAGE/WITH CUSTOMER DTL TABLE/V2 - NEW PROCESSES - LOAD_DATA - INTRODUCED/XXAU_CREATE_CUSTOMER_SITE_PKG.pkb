/* Formatted on 4/14/2026 1:47:46 PM (QP5 v5.365) */
...
IS
  ...
  * Parameters   :
    ...
    AS
    BEGIN
        UPDATE XXAU_CUSTOMER_DETAILS_TEST A
           SET ERROR_MSZ =
                   (SELECT ERP_ERR_MSG
            ...
            EXCEPTION
                ...
                THEN
                    ...
                    P_MSG :=
                           P_MSG
                        || ' - '
                        || 'Customer REC_ACCOUNT is not available in XXAU_CUSTOMER_DETAILS.';
            END;
        END IF;
    EXCEPTION
        WHEN OTHERS
  ...
  * Parameters   : P_ORGANIZATION_NAME   => IN PARAMETER , VARCHAR2
    ...
    BEGIN
        ...
        THEN
            ...
            DBMS_OUTPUT.PUT_LINE (
                               'x_account_number :' || P_ACCOUNT_NUMBER);
            DBMS_OUTPUT.PUT_LINE ( 'x_party_id :' || P_PARTY_ID);
            DBMS_OUTPUT.PUT_LINE (
                               'x_party_number :' || LC_PARTY_NUMBER);
            DBMS_OUTPUT.PUT_LINE (
                               'x_profile_id :' || P_PROFILE_ID);
        ...
        ELSE
            ...
            DBMS_OUTPUT.PUT_LINE ('p_msg complete...');
            ...
            THEN
                BEGIN
                    ...
                    FND_FILE.PUT_LINE (FND_FILE.LOG,'Inside Exception : CUST_ACCOUNTS');
                    
					DBMS_OUTPUT.PUT_LINE ('Inside Exception : CUST_ACCOUNTS');
    ...
    AS
        ...
        IS
            (SELECT A.AGENCY_ID,
                    ...
                    A.ERP_ERR_MSG,
                    A.CCID
               FROM AU_CUSTOMER_SHIP_TO_TEST A, XXAU_CUSTOMER_DETAILS_TEST B
              WHERE     A.AGENCY_ID IN
                            (SELECT AGENCY_ID
                               FROM XXAU_AGENCY_APPROVAL_DTL_TEST
                              WHERE     TRUNC (CREATION_DATE) >=
  ...
  * Parameters   :
    ...
    BEGIN
        ...
        FND_FILE.PUT_LINE (FND_FILE.LOG,
                           'GN_RESP_APPL_ID: ' || GN_RESP_APPL_ID);
        ...
        LOOP
            ...
            BEGIN
                ...
                UPDATE XXAU_CUSTOMER_DETAILS_TEST
     ...
     * Language     : PL/SQL
     * Description  : To insert the data into the XXAU_CUSTOMER_DETAILS_TEST table
     * History      -----------------------------------------------------------------------------------
  * Parameters   :
     *
     * WHO              Version #   WHEN            WHAT
    ...
    IS
        CURSOR CUR_LOAD IS
            (SELECT XXAU_INV_ORG.ORGANIZATION_CODE,
                    ...
                    NVL (
                        (SELECT ADDRESS
                           FROM AUCUSTOM.AU_CUSTOMER_MASTER_ADVANCE@EBSTOHR
                          WHERE     1 = 1
                                AND AGENCY_ID =
                                    DECODE (XXAU_AAD.AGENCY_ID,
                                            766849001, 7374551,
                                            766849002, 7471449,
                                            766849003, 8026294,
                                            766849004, 5939) --XXAU_AAD.AGENCY_ID
                                AND INV_ORG = XXAU_AAD.INV_ORG),
                        (SELECT ADDRESS
                           FROM AUCUSTOM.AU_CUSTOMER_MASTER_OCCUPA@EBSTOHR
                          WHERE     AGENCY_ID =
                                    DECODE (XXAU_AAD.AGENCY_ID,
                                            766849001, 7374551,
                                            766849002, 7471449,
                    ...
                    NVL (
                        (SELECT MOBILE_NO
                          ...
                          WHERE     AGENCY_ID =
                                ...
                                AND INV_ORG = XXAU_AAD.INV_ORG),
                        (SELECT MOBILE_NO
                           FROM AUCUSTOM.AU_CUSTOMER_AUTHORISE_PER@EBSTOHR
                          WHERE     AGENCY_ID =
                                    DECODE (XXAU_AAD.AGENCY_ID,
                                            766849001, 7374551,
                                            766849002, 7471449,
                    ...
                    NVL (
                        ...
                        PAN,
                    (SELECT STATE_CODE
                       FROM AUCUSTOM.XXAU_AGENCY_STATE@EBSTOHR
                    ...
                    SYSDATE
                        CREATION_DATE
               FROM --AUCUSTOM.XXAU_AGENCY_APPROVAL_DETIALS@EBSTOHR XXAU_AAD,
                    --AUCUSTOM.AU_CUSTOMER_SHIP_TO@EBSTOHR          AU_CUST_SHIP_TO,
                    --AUCUSTOM.XXAU_INV_ORGANIZATION@EBSTOHR        XXAU_INV_ORG,
                    --AUCUSTOM.AU_CUSTOMER_MASTER_NEW@EBSTOHR       AU_CUST_MST_NEW
                    XXAU_AGENCY_APPROVAL_DTL_TEST  XXAU_AAD,
                    AU_CUSTOMER_SHIP_TO_TEST       AU_CUST_SHIP_TO,
                    XXAU_INV_ORGANIZATION@EBSTOHR  XXAU_INV_ORG,
                    AU_CUSTOMER_MASTER_NEW_TEST    AU_CUST_MST_NEW
        ...
        LOOP
            BEGIN
                ...
                FORALL I IN CUR_TBL.FIRST .. CUR_TBL.LAST SAVE EXCEPTIONS
                    INSERT INTO XXAU_CUSTOMER_DETAILS_TEST (
                                    UNIT,
                                    UNIT_NAME,
                                    CUSTOMER_NAME,
                                    CUSTOMER_CLASS,
