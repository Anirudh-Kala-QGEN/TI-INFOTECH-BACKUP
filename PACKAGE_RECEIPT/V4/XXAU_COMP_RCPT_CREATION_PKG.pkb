CREATE OR REPLACE PACKAGE BODY XXAU_COMP_RCPT_CREATION_PKG
IS
	...
	BEGIN
		...
		INSERT INTO XXAU_AR_TRNSP_RCPT_ERR (CONC_RCPT_ID, COMPOSITE_RECEIPT_ID, STATUS,ERROR_MESSAGE) VALUES (P_CONS_RCPT_NO,P_COMPOSITE_RECEIPT_ID,P_PROCESS_FLAG, P_PROCESS_MESSAGE);
		
		IF P_COMPOSITE_RECEIPT_ID IS NULL
		THEN
			UPDATE XXAU_AR_TRNSP_RCPT_ERR
			SET STATUS = P_PROCESS_FLAG
			, ERROR_MESSAGE = ERROR_MESSAGE||'-'||P_PROCESS_MESSAGE
			WHERE 1=1
     ...
     * Language     : PL/SQL
     * Description  : This is for Updating the Stagging table with Error Message and the status
     * History      : 
	 * Parameters	: 
     *
     * WHO              Version #   WHEN            WHAT
	...
	BEGIN
		...
		UPDATE XXAU_AR_TRANS_ERP_RCPT_T A
		SET PROCESS_FLAG = (SELECT STATUS FROM XXAU_AR_TRNSP_RCPT_ERR WHERE COMPOSITE_RECEIPT_ID = A.COMPOSITE_RECEIPT_ID)
		, PROCESS_MESSAGE = PROCESS_MESSAGE||'-'||(SELECT ERROR_MESSAGE FROM XXAU_AR_TRNSP_RCPT_ERR WHERE COMPOSITE_RECEIPT_ID = A.COMPOSITE_RECEIPT_ID)
		WHERE 1=1
		AND COMPOSITE_RECEIPT_ID IS NOT NULL
	...
	CURSOR CUR_CONS_RCPT_NO IS -- MAIN BANK TRANSACTION; WHICH WILL HAVE THE SUM(PARTY_AMOUNT) OF SAME CONS_RCPT_NO
		...
		, BANK_ACC_ID
		, PROCESS_FLAG
		FROM XXAU_AR_TRANS_ERP_RCPT_T XATERT--ADDB.ADDB_XXA_AR_TRANS_ERP_RCPT@ebstohr
		WHERE 1=1
		AND (SELECT COUNT(1) 
					FROM XXAU_AR_TRANS_ERP_RCPT_T --ADDB.ADDB_XXA_AR_TRANS_ERP_RCPT@ebstohr
					WHERE CONS_RCPT_NO=XATERT.CONS_RCPT_NO) = (SELECT COUNT(1) 
																FROM XXAU_AR_TRANS_ERP_RCPT_T --ADDB.ADDB_XXA_AR_TRANS_ERP_RCPT@ebstohr
																WHERE PROCESS_FLAG = 'P' AND CONS_RCPT_NO=XATERT.CONS_RCPT_NO)
		AND CONS_RCPT_NO IS NOT NULL
		AND PROCESS_FLAG='P'
	...
	CURSOR RCPT_CUR(P_CONS_RCPT_NO VARCHAR2) IS -- REACIPT RECORDS FOR THE SAME CONS_RCPT_NO
	...
	LC_ERROR VARCHAR2(4000); -- LC_ERROR ADDB.ADDB_XXA_AR_TRANS_ERP_RCPT@ebstohr%TYPE;
	LC_DUMMY VARCHAR2(4000); -- LC_ERROR ADDB.ADDB_XXA_AR_TRANS_ERP_RCPT@ebstohr%TYPE;
	...
	P_RESP_APPL_ID NUMBER := FND_PROFILE.VALUE('RESP_APPL_ID');
	...
	MO_GLOBAL.INIT('AR');
	FND_GLOBAL.APPS_INITIALIZE(USER_ID 		=> NVL(P_USER_ID,2482)
	...
	DBMS_OUTPUT.PUT_LINE('--- INSIDE ENTER_RECORDS ---');
	...
	L_ATTRIBUTE_REC.ATTRIBUTE15 			:= NULL;
	LC_COMMENTS_H:=NULL;
		...
		LOOP
			BEGIN
				...
				L_CUST_ID := NULL;
				J.PROCESS_FLAG := 'E';
				LC_ERROR:=LC_ERROR||'; Error in customer:'||J.ERP_CUSTOMER_NUMBER||'; '||SQLCODE||'-'||SQLERRM||'; ';
				DBMS_OUTPUT.PUT_LINE('Customer ID fetching error FOR : CUSTOMER NUMBER'||J.ERP_CUSTOMER_NUMBER||'; '||SQLCODE||'-'||SQLERRM||'; ');
				END;
				
				-- FOR DUMMY BANK ACCORDING TO BANK_ACC_NAME
				...
				EXCEPTION WHEN OTHERS THEN
					...
					DBMS_OUTPUT.PUT_LINE('Receipt Method ID fetching error FOR BANK_ACC_NAME: '||J.BANK_ACC_NAME||'; '||SQLCODE||'-'||SQLERRM||'; ');
				END;
								
				DBMS_OUTPUT.PUT_LINE('CONC RCPT NO : '||I.CONS_RCPT_NO
													  ||' . '||LN_INT_RCPT_NO
													  ||' : Data  - ' || J.PARTY_NUMBER
													  ||' - '|| J.PARTY_NAME
													  ||' - '|| J.PARTY_LOCATION
				...
				-- Call AR Receipt API for different customers under same bank
						AR_RECEIPT_API_PUB.CREATE_CASH(
										...
										-- Mandatory Fields
										p_receipt_number => TO_CHAR('C'||I.CONS_RCPT_NO||'.'||LN_INT_RCPT_NO), -- invoice_num -> 
						...
						DBMS_OUTPUT.PUT_LINE('JUST AFTER LINE''S API L_MSG_DATA : '||l_msg_data);			  
		...
		DBMS_OUTPUT.PUT_LINE('LC_COMMENTS_H : '||LC_COMMENTS_H||' FOR RECEIPT NUM :'||'C'||I.CONS_RCPT_NO||' BECAUSE OF '||LC_ERROR);
		...
		THEN
			...
			LC_FLAG_H:='E';			
			L_MSG_DATA_R := L_MSG_DATA_R||' RECEIVABLES TRANSACTION IS NOT VALID';
			END;
			...
			BEGIN
				...
				WHERE BANK_ACCOUNT_ID = I.BANK_ACC_ID;
			EXCEPTION
			WHEN OTHERS THEN
			REMIT_BANK_ACCT_USE_ID := NULL;
			LC_FLAG_H:='E';	
			L_MSG_DATA_R :=L_MSG_DATA_R|| ' REMIT BANK NOT VALID';
			END;
			
			DBMS_OUTPUT.PUT_LINE('LC_FLAG_H : '||LC_FLAG_H||' FOR RECEIPT NUM :'||'C'||I.CONS_RCPT_NO||' BECAUSE OF '||L_MSG_DATA_R);
			IF LC_FLAG_H <> 'E'
			THEN
				--FND_MSG_PUB.INITIALIZE;
				DBMS_OUTPUT.PUT_LINE('INSIDE API FOR: '||'C'||I.CONS_RCPT_NO);
				L_CASH_RECEIPT_NUM:='C'||I.CONS_RCPT_NO;
				DBMS_OUTPUT.PUT_LINE('L_CASH_RECEIPT_NUM : '||L_CASH_RECEIPT_NUM||' LN_RECEIVABLES_TRX_ID: '||LN_RECEIVABLES_TRX_ID||' FOR LC_RECE_TRX_NAME :'||LC_RECE_TRX_NAME);
				AR_RECEIPT_API_PUB.CREATE_MISC
					...
					DBMS_OUTPUT.PUT_LINE('x_msg_count: '||l_msg_count||' l_return_status :'||l_return_status||' l_msg_data : '||l_msg_data);
					IF l_return_status <> FND_API.G_RET_STS_SUCCESS 
					THEN
						FOR i IN 1 .. l_msg_count LOOP
							FND_MSG_PUB.GET(p_msg_index => FND_MSG_PUB.G_NEXT
					...
					ELSIF l_return_status = FND_API.G_RET_STS_SUCCESS
						THEN 
							...
							, PROCESS_MESSAGE = PROCESS_MESSAGE||' - SUCCESS; RECEIPT NO. : '||L_CASH_RECEIPT_NUM
							WHERE 1=1
							AND CONS_RCPT_NO = I.CONS_RCPT_NO;
							
							PRAGMA_COMMIT('S',' - SUCCESS; RECEIPT NO. : '||L_CASH_RECEIPT_NUM,I.CONS_RCPT_NO ,NULL );
							
						COMMIT; -- COMMIT ALL RECORDS
					END IF;
			ELSE
			PRAGMA_COMMIT('E','Error COMP RCPT: '||L_MSG_DATA_R,I.CONS_RCPT_NO ,NULL );
			ROLLBACK;
			END IF;
		END IF;
	...
	EXCEPTION
	WHEN OTHERS THEN
	DBMS_OUTPUT.PUT_LINE('Error in ENTER_RECORDS Procedure: '||SQLCODE||' ' ||SQLERRM);
	END ENTER_RECORDS;
	...
	BEGIN
	DBMS_OUTPUT.PUT_LINE('INSIDE VALIDATION_REQUIRED ....... ');
		...
		LOOP
			...
			BEGIN
				...
				-- FOR DUMMY BANK ACCORDING TO ATTRIBUTE7
				LC_DUMMY := 'DUMMY BANK '||REGEXP_REPLACE(J.BANK_ACC_NAME, '\s*cash', '', 1, 0, 'i');
				...
				-- VALIDATING THE RECEIVABLES_TRX_ID FOR ATTRIBUTE9
				LC_RECE_TRX_NAME:= SUBSTR(J.ATTRIBUTE9,1,3)||'%MISC%RECEIPTS';
				BEGIN
					SELECT RECEIVABLES_TRX_ID
					INTO LN_RECEIVABLES_TRX_ID
				...
				WHEN OTHERS THEN
					REMIT_BANK_ACCT_USE_ID := NULL;
					J.PROCESS_FLAG := 'E';
					LC_ERROR:=LC_ERROR||'REMIT_BANK_ACCT_USE_ID error: '||REMIT_BANK_ACCT_USE_ID||'; '||SQLCODE||'-'||SQLERRM||'; ';
					DBMS_OUTPUT.PUT_LINE('REMIT_BANK_ACCT_USE_ID fetching error FOR REMIT_BANK_ACCT_USE_ID AT BANK_ACC_ID: '||J.BANK_ACC_ID||'; '||SQLCODE||'-'||SQLERRM||'; ');
				END;
			...
			EXCEPTION
				WHEN OTHERS THEN
					...
					LC_ERROR:=LC_ERROR||' - '||SQLCODE||'-'||SQLERRM||'; ';
		...
		END LOOP;
	EXCEPTION WHEN OTHERS THEN
	DBMS_OUTPUT.PUT_LINE('Error in VALIDATION_REQUIRED Procedure: '||SQLCODE||' ' ||SQLERRM||'; ');
	...
	AS
	BEGIN
		--DBMS_OUTPUT.PUT_LINE('Calling VALIDATION_REQUIRED');
		...
		DBMS_OUTPUT.PUT_LINE('Calling ENTER_RECORDS');
		ENTER_RECORDS;
		
		
		DBMS_OUTPUT.PUT_LINE('------------------------------------------------');
		DBMS_OUTPUT.PUT_LINE('Calling AFTER_PROCESS');
		AFTER_PROCESS;
		
	EXCEPTION WHEN OTHERS THEN
	DBMS_OUTPUT.PUT_LINE('Error in MAIN Procedure: '||SQLCODE||' ' ||SQLERRM||'; ');
	END MAIN;
