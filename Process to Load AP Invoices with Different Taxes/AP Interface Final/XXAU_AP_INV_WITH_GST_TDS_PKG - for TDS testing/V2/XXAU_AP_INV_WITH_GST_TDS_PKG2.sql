...
IS
	 ...
	 * AKALA            1.4         08-MAY-2026      Updated code for loop name, user exception and continue																											
	 ***************************************************************************************************/
...
---------------------------------------------------------------------------------------------------------------------------------
     ...
     * AKALA            1.2         08-MAY-2026     Added pragma exception and Continue statement to remove bug																											   
     ***************************************************************************************************/
...
PROCEDURE LOAD_DATA_TO_INTERFACE (P_BATCH_ID NUMBER)
	...
	USER_EXP EXCEPTION;							-- Added by AKALA ON 08-MAY-2026
	PRAGMA EXCEPTION_INIT(USER_EXP, -20001);	-- Added by AKALA ON 08-MAY-2026
	...
	BEGIN
		...
		<<HEADER_LOOP>>	-- Added by AKALA ON 08-MAY-2026										  
		FOR REC_HDR IN CUR_HDR_DATA
		LOOP
			BEGIN
				...
				LN_LINE_NUMBER := 1; 
				<<LINE_LOOP>>		-- Added by AKALA ON 08-MAY-2026												   
				FOR REC_LINE IN CUR_LINE_DATA  (REC_HDR.INVOICE_NUMBER  ,REC_HDR.OPERATING_UNIT, REC_HDR.PARTY_NAME)
				LOOP
					BEGIN     
						...
						<<TAX_LOOP>>		-- Added by AKALA ON 08-MAY-2026						
						...
						LOOP
							BEGIN
								...
								BEGIN
								FND_FILE.PUT_LINE(FND_FILE.LOG,'FETCHING INTERIM_RECOVERY_CCID');		-- Added by AKALA ON 08-MAY-2026
								...
								EXCEPTION
									WHEN OTHERS
									THEN
										FND_FILE.PUT_LINE(FND_FILE.LOG,'EXCEPTION INTERIM_RECOVERY_CCID');
										LN_LIABILITY_CCID := NULL;
										ROLLBACK;	-- Added by AKALA ON 08-MAY-2026
										FND_FILE.PUT_LINE(FND_FILE.LOG,'ROLLBACK COMPLETED');
										
										-- Added by AKALA ON 08-MAY-2026
										UPDATE XXAU_AP_INV_WITH_GST_TDS_TBL@ebstohr
										SET PROCESS_FLAG = 'E'
											, ERROR_MSG = ERROR_MSG||HDR_ERR_MSG|| ';INTERIM_RECOVERY_CCID is not available in JAI_TAX_ACCOUNTS_V for Operating Unit: '||REC_LINE.OPERATING_UNIT||' Inventory Org :'||REC_LINE.INV_ORGANIZATION_ID||'; Tax type code: '||REC_TAXES.TAX_TYPE_CODE
											, UPDATED_BY = GN_USER_ID
											, LAST_UPDATE_DATE = SYSDATE
										WHERE 1=1
										...
										;
										COMMIT;
										--FND_FILE.PUT_LINE(FND_FILE.LOG,'COMMIT COMPLETED');
										RAISE USER_EXP; 	-- Added by AKALA ON 08-MAY-2026
								END;
							...
							EXCEPTION
							WHEN USER_EXP	-- Added by AKALA ON 08-MAY-2026, LOOP NAME
							THEN
								--FND_FILE.PUT_LINE(FND_FILE.LOG,'TRANSFERRING THE LOOP FLOW TO HEADER LOOP');
								CONTINUE header_loop;	-- Added by AKALA ON 08-MAY-2026
							WHEN OTHERS THEN
								FND_FILE.PUT_LINE(FND_FILE.LOG,SQLCODE||' - '||SQLERRM); 
							END;
						END LOOP TAX_LOOP;	-- Added by AKALA ON 08-MAY-2026, LOOP NAME
					...
					--FND_FILE.PUT_LINE(FND_FILE.LOG,SQLCODE||' - '||SQLERRM);
					 UPDATE XXAU_AP_INV_WITH_GST_TDS_TBL@ebstohr
						...
						;
					END;
				END LOOP LINE_LOOP;	-- Added by AKALA ON 08-MAY-2026, LOOP NAME
			EXCEPTION
				WHEN OTHERS THEN
					FND_FILE.PUT_LINE(FND_FILE.LOG,SQLCODE||' - '||SQLERRM);
				...
				COMMIT;
			END;
		END LOOP HEADER_LOOP;	-- Added by AKALA ON 08-MAY-2026, LOOP NAME
	...
	AND EXISTS  (SELECT 1
							   ...
							   WHERE     C.VENDOR_CODE = B.VENDOR_CODE
									 AND C.UNIT 	 = B.UNIT
									 AND C.VENDOR_SITE_CODE = B.VENDOR_SITE_CODE
									 AND C.SET_OF_BOOKS_ID = (SELECT SET_OF_BOOKS_ID
	...
	BEGIN
		...
		LOOP
			BEGIN
				...
				ELSE
					UPDATE XXAU_AP_INV_WITH_GST_TDS_TBL@ebstohr
					SET ERROR_MSG = ERROR_MSG ||' '||LC_HDR_ERROR_MSG
					, PROCESS_FLAG = LC_HDR_PRC_FLAG
					, ERP_ORG_ID = REC_HDR.ERP_ORG_ID
					, ERP_VENDOR_ID = REC_HDR.ERP_VENDOR_ID
					, LEDGER_ID = REC_HDR.LEDGER_ID
					, INVOICE_TYPE_LOOKUP_CODE = REC_HDR.INVOICE_TYPE_LOOKUP_CODE
					, UPDATED_BY = GN_USER_ID
					, LAST_UPDATE_DATE = SYSDATE
					WHERE INVOICE_NUMBER = REC_HDR.INVOICE_NUMBER
					AND OPERATING_UNIT = REC_HDR.OPERATING_UNIT
					AND PARTY_NAME = REC_HDR.PARTY_NAME
		...
		LOOP
			BEGIN
				...
				THEN
					...
					END LOOP;
				ELSE 
				-- Added by AKALA on 08-MAY-2026, Due to multiple vendor site issue.
					UPDATE XXAU_AP_INV_WITH_GST_TDS_TBL@ebstohr
					SET ERROR_MSG = ERROR_MSG ||' '||LC_HDR_ERROR_MSG
					, PROCESS_FLAG = LC_HDR_PRC_FLAG
					, ERP_ORG_ID = REC_HDR.ERP_ORG_ID
