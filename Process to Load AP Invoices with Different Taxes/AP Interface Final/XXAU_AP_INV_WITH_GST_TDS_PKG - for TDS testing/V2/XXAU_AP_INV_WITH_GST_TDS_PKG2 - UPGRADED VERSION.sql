...
---------------------------------------------------------------------------------------------------------------------------------
	...
	PROCEDURE UPDATE_DISTRIBUTION(P_BATCH_ID IN NUMBER)
	AS 
	CURSOR INV_DATA
	IS (
	SELECT *
	FROM XXAU_AP_INV_WITH_GST_TDS_TBL A
	WHERE 1=1
	AND EXISTS (SELECT 1 FROM AP_INVOICES_ALL WHERE INVOICE_NUM = A.INVOICE_NUMBER AND VENDOR_ID = A.ERP_VENDOR_ID AND VENDOR_SITE_ID = A.VENDOR_SITE_ID)
	AND ERP_INVOICE_ID IS NOT NULL
	AND PROCESS_FLAG = 'S'
	...
	BEGIN
		...
		LOOP
			BEGIN
				...
				FND_FILE.PUT_LINE(FND_FILE.LOG,'UPDATING DISTRIBUTION AND LINES IN ERP');
				FND_FILE.PUT_LINE(FND_FILE.LOG,'I.ERP_INVOICE_ID :'||I.ERP_INVOICE_ID||'; I.ERP_CCID: '||I.ERP_CCID||'; I.GROUP_LINE_NUM : '||I.GROUP_LINE_NUM);
				
				UPDATE AP_INVOICE_DISTRIBUTIONS_ALL
				SET DIST_CODE_COMBINATION_ID = I.ERP_CCID
				WHERE INVOICE_ID = I.ERP_INVOICE_ID
				AND INVOICE_LINE_NUMBER = I.GROUP_LINE_NUM
				AND ORG_ID = I.ERP_ORG_ID
				;
				
				UPDATE AP_INVOICE_LINES_ALL
				SET DEFAULT_DIST_CCID = NULL
				WHERE INVOICE_ID = I.ERP_INVOICE_ID
				AND LINE_NUMBER = I.GROUP_LINE_NUM
				AND ORG_ID = I.ERP_ORG_ID
				;
     ...
     * AKALA            1.2         07-MAY-2026     Commented the cursor which are not in use.
     ***************************************************************************************************/
...
PROCEDURE LOAD_DATA_TO_INTERFACE (P_BATCH_ID NUMBER)
	...
	(SELECT   BATCH_ID
			...
			, ERP_ORG_ID
			, SUM(LINE_AMOUNT) INVOICE_AMOUNT
	FROM XXAU_AP_INV_WITH_GST_TDS_TBL B
	WHERE 1=1
	AND BATCH_ID = P_BATCH_ID
			...
			BEGIN
				...
				INSERT INTO AP_INVOICES_INTERFACE (
                     ...
                     VALUES (LN_INVOICE_ID,
                             ...
                             'CHECK',
                             'AP_INDUS_API',
							 REC_HDR.DOCUMENT_CATEGORY_CODE)
							 ;
					...
					BEGIN     
						...
						INSERT INTO AP_INVOICE_LINES_INTERFACE (
						 ...
						 VALUES (LN_INVOICE_ID,
								 ...
								 SYSDATE,
								 GN_USER_ID,
								 REC_HDR.ACCTS_PAY_CC_ID, --REC_LINE.ERP_CCID,
								 REC_HDR.INVOICE_NUMBER,
								 LN_LINE_NUMBER,
								 'OFI TAX IMPORT',
								 REC_LINE.CONTEXT_VALUE,
	...
	BEGIN
		...
		LOOP
			BEGIN
				...
				-- VALIDATING DOCUMENT CATEGORY -- Added by AKALA , on 07-MAY-2026
				BEGIN
					SELECT CODE
					INTO REC_HDR.DOCUMENT_CATEGORY_CODE
					from FND_DOC_SEQUENCE_CATEGORIES 
					WHERE 1=1
					AND UPPER(NAME) = TRIM(UPPER(REC_HDR.DOCUMENT_CATEGORY_NAME))
					AND TABLE_NAME = 'AP_INVOICES_ALL'
					AND APPLICATION_ID = 200;
				EXCEPTION
				WHEN OTHERS THEN
					REC_HDR.DOCUMENT_CATEGORY_CODE:=NULL; 
					LC_HDR_PRC_FLAG := 'E';
					LC_HDR_ERROR_MSG := REG_ERR_MSG (LC_HDR_ERROR_MSG,' - Document Category Error.');
				...
				LOOP
					...
					*/	
					...
					SET ERP_VENDOR_ID = REC_HDR.ERP_VENDOR_ID
						...
						, DOCUMENT_CATEGORY_CODE = REC_HDR.DOCUMENT_CATEGORY_CODE
     ...
     * ===============  =========   =============   ====================================================
     * AKALA            1.0         23-APR-2026     Initial Version
     * AKALA            1.1         07-MAY-2026     Added document category for invoice header.
     ***************************************************************************************************/
	...
	BEGIN
		...
		UPDATE_DISTRIBUTION(P_BATCH_ID);
		FND_FILE.PUT_LINE(FND_FILE.LOG,'Distribution Updated..');
		ELSE
			FND_FILE.PUT_LINE(FND_FILE.LOG,'Please enter the batch no..');
		END IF;
