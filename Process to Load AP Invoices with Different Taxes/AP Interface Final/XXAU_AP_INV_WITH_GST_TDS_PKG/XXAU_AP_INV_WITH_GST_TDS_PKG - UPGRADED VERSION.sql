CREATE OR REPLACE PACKAGE BODY APPS.XXAU_AP_INV_WITH_GST_TDS_PKG
IS
    /*************************************************************************************************
...
---------------------------------------------------------------------------------------------------------------------------------
    ...
    /*************************************************************************************************
     * Program Name : UPDATE_DISTRIBUTION
     * Language     : PL/SQL
     * Description  : This a local function is used for Message formatting.
     * Parameters : 1. P_BATCH_ID => NUMBER
     *
     * History      :
     * WHO              Version #   WHEN            WHAT
     * ===============  =========   =============   ====================================================
     * AKALA            1.0         08-MAY-2026     Initial Version
     ***************************************************************************************************/
...
PROCEDURE LOAD_DATA_TO_INTERFACE (P_BATCH_ID NUMBER)
					...
					BEGIN     
						...
						INSERT INTO AP_INVOICE_LINES_INTERFACE (
						 ...
						 VALUES (LN_INVOICE_ID,
								 ...
								 SYSDATE,
								 GN_USER_ID,
								 REC_HDR.ACCTS_PAY_CC_ID, --REC_LINE.ERP_CCID, Commented on 08-MAY-2026,
								 REC_HDR.INVOICE_NUMBER,
								 LN_LINE_NUMBER,
								 'OFI TAX IMPORT',
								 REC_LINE.CONTEXT_VALUE,
	...
	AS
		BEGIN
			...
			THEN
				...
				UPDATE_DISTRIBUTION(P_BATCH_ID);	-- To update the ERP Distribution CCID and Line CCID
			ELSE
	...
	BEGIN
		...
		SHOW_DATA (P_BATCH_ID);					-- Update the status on Custom table and shows all invoices created in ERP
		VALIDATE_INV_IN_ERP(P_BATCH_ID); 		-- Validation program submission for distribution creation
		FND_FILE.PUT_LINE(FND_FILE.LOG,'UPDATING DISTRIBUTIONS...');
		UPDATE_DISTRIBUTION(P_BATCH_ID);		-- To update the ERP Distribution CCID and Line CCID
		ELSE
			FND_FILE.PUT_LINE(FND_FILE.LOG,'Please enter the batch no..');
		END IF;
