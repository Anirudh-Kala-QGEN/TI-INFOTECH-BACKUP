...
PROCEDURE LOAD_DATA_TO_INTERFACE (P_BATCH_ID NUMBER)
				...
				LOOP
					BEGIN     
						...
						LOOP
							...
							END;
						END LOOP;
						/* -- Commented by AKALA on 28-APR-2026, No need to work for TCS
						LN_TAX_TCS_LINE_COUNTER := LN_TAX_LINE_COUNTER;
						IF LN_TAX_LINE_COUNTER < 10
						THEN
							FOR REC_TAXES IN CUR_TAXES (REC_LINE.TCS
														, NULL--REC_LINE.TAX_RATE_NAME 
														, NULL--REC_LINE.INVOICE_TAX_TYPE
														, REC_LINE.ERP_ORG_ID)
							LOOP
								BEGIN
									...
									DBMS_OUTPUT.PUT_LINE ('TAX_LINE_NO = LN_TAX_TCS_LINE_COUNTER : '|| LN_TAX_TCS_LINE_COUNTER);
									
									-- Tax Liability CCID

									BEGIN
										SELECT INTERIM_LIABILITY_CCID --INTERIM_RECOVERY_CCID --RECOVERY_CCID--LIABILITY_CCID
										  INTO LN_LIABILITY_CCID
										  FROM JAI_TAX_ACCOUNTS_V
										 WHERE     ORG_ID = REC_LINE.ERP_ORG_ID
											   AND ORGANIZATION_ID = NVL (REC_LINE.INV_ORGANIZATION_ID,REC_LINE.ERP_ORG_ID)
									...
									INSERT INTO JAI_INTERFACE_TAX_LINES_ALL (
										 ...
										 VALUES (REC_HDR.ERP_VENDOR_ID,
												 ...
												 LN_LIABILITY_CCID,
												 0,
												 (CASE WHEN LN_TAX_TCS_LINE_COUNTER >= 2 THEN 1 END),
												 (CASE WHEN LN_TAX_TCS_LINE_COUNTER >= 3 THEN 2 END),
												 (CASE WHEN LN_TAX_TCS_LINE_COUNTER >= 4 THEN 3 END),
												 (CASE WHEN LN_TAX_TCS_LINE_COUNTER >= 5 THEN 4 END),
												 (CASE WHEN LN_TAX_TCS_LINE_COUNTER >= 6 THEN 5 END),
												 (CASE WHEN LN_TAX_TCS_LINE_COUNTER >= 7 THEN 6 END),
												 (CASE WHEN LN_TAX_TCS_LINE_COUNTER >= 8 THEN 7 END),
												 (CASE WHEN LN_TAX_TCS_LINE_COUNTER >= 9 THEN 8 END),
												 (CASE WHEN LN_TAX_TCS_LINE_COUNTER = 10 THEN 9 END)
												 );
									COMMIT;
																		
									LN_TAX_TCS_LINE_COUNTER := LN_TAX_TCS_LINE_COUNTER + 1;
								EXCEPTION WHEN OTHERS THEN
									FND_FILE.PUT_LINE(FND_FILE.LOG,SQLCODE||' - '||SQLERRM);
								END;
							END LOOP;
						ELSE 
							FND_FILE.PUT_LINE(FND_FILE.LOG,'Cannot set Precedence for TCS Lines.');
						END IF ; */
			...
			BEGIN
				...
				LOOP
					...
					WHEN OTHERS THEN
						...
						UPDATE XXAU_AP_INV_WITH_GST_TDS_TBL
						SET PROCESS_FLAG = 'E'
						, ERROR_MSG = 'Error while inserting into Interface.'
						WHERE RECORD_ID = REC_LINE.RECORD_ID
						AND OPERATING_UNIT = REC_LINE.OPERATING_UNIT
						AND INVOICE_NUMBER = REC_HDR.INVOICE_NUMBER
						;
	...
	AS
		BEGIN
			...
			THEN
				VALIDATE_DATA (P_BATCH_ID); 		-- Validate all data for specific Batch ID
