    ...
    BEGIN
        ...
        -- FOR VALIDATING VALUE OF ATTRIBUTE7 WITH VALUESET ATTACHED ON AR_CASH_RECEIPTS_ALL.ATTRIBUTE7 -- ADDDED ON 12-FEB-2026 BY AKALA
						...
						OR (STRUCTURED_HIERARCHY_LEVEL IN
						  (SELECT HIERARCHY_ID
							...
							WHERE     H.FLEX_VALUE_SET_ID = (SELECT FLEX_VALUE_SET_ID
															FROM FND_FLEX_VALUE_SETS A --1022397
															WHERE FLEX_VALUE_SET_NAME =  'XXA_BANK_ACCOUNT_NUMBER')
															AND H.HIERARCHY_NAME LIKE '')))
						AND (FLEX_VALUE_SET_ID = (SELECT FLEX_VALUE_SET_ID
													FROM FND_FLEX_VALUE_SETS A --1022397
													WHERE FLEX_VALUE_SET_NAME =  'XXA_BANK_ACCOUNT_NUMBER'))
						AND (UPPER(PARENT_FLEX_VALUE_LOW) = UPPER(TRIM(J.ATTRIBUTE8)))
					EXCEPTION
					WHEN OTHERS THEN
					 J.ATTRIBUTE8 := NULL;
					 J.PROCESS_FLAG := 'E';
			...
			BEGIN
				SELECT CHECK_TYPE
				INTO J.ATTRIBUTE2
				FROM AUCUSTOM.CHECK_LOCAL@EBSTOHR 
				WHERE CHECK_TYPE = TRIM(J.ATTRIBUTE2);
			EXCEPTION WHEN OTHERS THEN
			 J.ATTRIBUTE2 := NULL;
			 J.PROCESS_FLAG := 'E';
            ...
            BEGIN
				...
				AND LOOKUP_CODE = TRIM(J.ATTRIBUTE3);
			EXCEPTION WHEN OTHERS THEN
			 J.ATTRIBUTE3 := NULL;
			 J.PROCESS_FLAG := 'E';
            ...
            BEGIN
				SELECT LOOKUP_CODE
				 ...
				 FROM FND_LOOKUPS
				WHERE LOOKUP_TYPE='YES_NO'
				 AND LOOKUP_CODE = TRIM(ATTRIBUTE4);
			EXCEPTION WHEN OTHERS THEN
			 J.ATTRIBUTE4 := NULL;
			 J.PROCESS_FLAG := 'E';
			...
			BEGIN
				SELECT LOOKUP_CODE
				 ...
				 FROM FND_LOOKUPS
				WHERE LOOKUP_TYPE='YES_NO'
				 AND LOOKUP_CODE = TRIM(ATTRIBUTE5);
			EXCEPTION WHEN OTHERS THEN
			 J.ATTRIBUTE5 := NULL;
			 J.PROCESS_FLAG := 'E';
			...
			BEGIN
			 SELECT LOOKUP_CODE
				 ...
				 FROM FND_LOOKUPS
				WHERE LOOKUP_TYPE='YES_NO'
				 AND LOOKUP_CODE = TRIM(ATTRIBUTE6);
			EXCEPTION WHEN OTHERS THEN
			 J.ATTRIBUTE6 := NULL;
			 J.PROCESS_FLAG := 'E';
