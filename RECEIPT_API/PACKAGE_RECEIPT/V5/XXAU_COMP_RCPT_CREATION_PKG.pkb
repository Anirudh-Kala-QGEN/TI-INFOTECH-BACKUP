/* Formatted on 2/13/2026 10:58:00 AM (QP5 v5.365) */
...
IS
  ...
  *      4. P_COMPOSITE_RECEIPT_ID => COMPOSITE_RECEIPT_ID
    ...
    BEGIN
        ...
        THEN
            UPDATE XXAU_AR_TRNSP_RCPT_ERR
               SET STATUS = P_PROCESS_FLAG, ERROR_MESSAGE = P_PROCESS_MESSAGE
             WHERE     1 = 1
	...
	EXCEPTION
	WHEN OTHERS THEN
	DBMS_OUTPUT.PUT_LINE('Error in PRAGMA_COMMIT : '||SQLCODE||'-'||SQLERRM);
    END PRAGMA_COMMIT;
    ...
    BEGIN
        ...
        COMMIT;
		DBMS_OUTPUT.PUT_LINE(RPAD('-',20,'-')||CHR(10)||'Process Complete');
	EXCEPTION
	WHEN OTHERS THEN
	DBMS_OUTPUT.PUT_LINE('Error in AFTER_PROCESS : '||SQLCODE||'-'||SQLERRM);
    END AFTER_PROCESS;
  ...
  * Parameters :
    ...
    BEGIN
                ...
                BEGIN
                    ...
                    THEN
                        ...
                        AR_RECEIPT_API_PUB.CREATE_CASH (
                            ...
                            p_cr_id               => l_cr_id);
                        DBMS_OUTPUT.PUT_LINE (
                               'JUST AFTER LINE''S API L_MSG_DATA : '
                            || l_msg_data);
       ...
       END LOOP;*/
                        ...
                        THEN
                            LN_TOTAL_RCPT_AMT :=
                                LN_TOTAL_RCPT_AMT + J.PARTY_AMT;
                            LC_RECE_TRX_NAME :=
                                   SUBSTR (J.ATTRIBUTE9, 1, 3)
                                || '%MISC%RECEIPTS';

                            BEGIN
                                SELECT DECODE (
                                           NVL (LENGTH (LC_COMMENTS_H), 0),
                                           0, '',
                                           ', ')
                                  INTO P_SEPRATER
                                  FROM DUAL;
            ...
            THEN
                ...
                EXCEPTION
                    ...
                    THEN
                        ...
                        L_MSG_DATA_R :=
                               L_MSG_DATA_R
                            || ' RECEIVABLES TRANSACTION IS NOT VALID';
                END;
                ...
                EXCEPTION
                    ...
                    THEN
                        ...
                        LC_FLAG_H := 'E';
                        L_MSG_DATA_R :=
                            L_MSG_DATA_R || ' REMIT BANK NOT VALID';
                END;
                ...
                THEN
                    --FND_MSG_PUB.INITIALIZE;
                    DBMS_OUTPUT.PUT_LINE (
                        'INSIDE API FOR: ' || 'C' || I.CONS_RCPT_NO);
                    ...
                    THEN
                        ...
                        PRAGMA_COMMIT (
                            'E',
                               'Error while creating MISC Composite Receipt'
                            || L_MSG_DATA_R
                            || ' '
                            || LC_DATA,
                            I.CONS_RCPT_NO,
 ...
 */
                ...
                LC_RECE_TRX_NAME :=
                    SUBSTR (J.ATTRIBUTE9, 1, 3) || '%MISC%RECEIPTS';
