...
IS
  ...
  *      4. P_COMPOSITE_RECEIPT_ID => COMPOSITE_RECEIPT_ID
    ...
    BEGIN
        ...
        /* -- Commented by AKALA on 01-MAY-2026; to jsut store every line with their own process message.
	 ...
	 * AKALA			1.1			30-MAR-2026 	Added the updated by, process_date, updation_date for Update
     ***************************************************************************************************/

    PROCEDURE AFTER_PROCESS
    AS
	LN_LAST_UPDATED_BY NUMBER := FND_PROFILE.VALUE('USER_ID');
    BEGIN
        /* -- Commented by AKALA due to a bug on 17-MAR-2026
		UPDATE XXAU_AR_TRANS_ERP_RCPT_T XATERT -- ADDB.ADDB_XXA_AR_TRANS_ERP_RCPT@ebstohr A --XXAU_AR_TRANS_ERP_RCPT_T
           SET PROCESS_FLAG =
                   (SELECT STATUS
		...
		*/
		...
		UPDATE XXAU_AR_TRANS_ERP_RCPT_T A -- ADDB.ADDB_XXA_AR_TRANS_ERP_RCPT@ebstohr  A --XXAU_AR_TRANS_ERP_RCPT_T
           SET PROCESS_FLAG =
                   (SELECT STATUS
                         FROM XXAU_AR_TRNSP_RCPT_ERR
         ...
         WHERE     1 = 1
               AND COMPOSITE_RECEIPT_ID IS NOT NULL
               AND CONS_RCPT_NO IS NOT NULL
			   AND EXISTS (SELECT 1 FROM XXAU_AR_TRNSP_RCPT_ERR WHERE COMPOSITE_RECEIPT_ID = A.COMPOSITE_RECEIPT_ID AND CONC_RCPT_ID = A.CONS_RCPT_NO)
			   ;
  ...
  * Parameters :
	 ...
	 * AKALA			1.3			30-MAR-2026		Added P_UNIT parameter and Changed P_Receipt_Date and P_gl_Date to Sysdate for all APIs
     ***************************************************************************************************/

    PROCEDURE ENTER_RECORDS(P_UNIT IN NUMBER)
    AS
        CURSOR CUR_CONS_RCPT_NO IS -- MAIN BANK TRANSACTION; WHICH WILL HAVE THE SUM(PARTY_AMOUNT) OF SAME CONS_RCPT_NO
              SELECT CONS_RCPT_NO,
                     ...
                     BANK_ACC_ID, 
                     PROCESS_FLAG
                FROM XXAU_AR_TRANS_ERP_RCPT_T XATERT -- ADDB.ADDB_XXA_AR_TRANS_ERP_RCPT@ebstohr XATERT --XXAU_AR_TRANS_ERP_RCPT_T
               WHERE     1 = 1
                     AND (SELECT COUNT (1)
                            FROM XXAU_AR_TRANS_ERP_RCPT_T XATERT -- ADDB.ADDB_XXA_AR_TRANS_ERP_RCPT@ebstohr  --XXAU_AR_TRANS_ERP_RCPT_T
                     ...
                     AND CONS_RCPT_NO IS NOT NULL
                     AND PROCESS_FLAG = 'P'
					 AND EXISTS (SELECT 1 FROM xxau_accounting_unit_new@ebstohr
								   WHERE ORGANIZATION_CODE NOT IN ('HSO')
									 AND ORGANIZATION_CODE =SUBSTR(XATERT.ATTRIBUTE11,1,3)
									 --AND FLEX_VALUE = P_UNIT 
									 AND ORGANIZATION_ID = P_UNIT 
									 )
    ...
    BEGIN
        ...
        LOOP
            ...
            LC_COMMENTS_H := NULL;
			LC_FLAG_H := 'S'; -- Added by AKALA on 01-MAY-2026
            ...
            LOOP
                BEGIN
					...
					EXCEPTION 
						...
						J.PROCESS_FLAG := 'E';
						LC_FLAG_H := 'E'; -- Added by AKALA on 01-MAY-2026
                ...
                BEGIN
                    ...
                    THEN
                        ...
                        AR_RECEIPT_API_PUB.CREATE_CASH (
                            ...
                            p_receipt_date        => TRUNC (SYSDATE), --J.RECEIPT_DATE,
                            p_gl_date             => TRUNC (SYSDATE), --J.RECEIPT_DATE,
            ...
            IF NVL (LENGTH (TRIM (REPLACE(LC_COMMENTS_H,',',' '))), 0) <> 0
            THEN
                -- LC_FLAG_H := 'S'; --Comment by AKALA on 01-MAY-2026
                ...
                THEN
					...
					AR_RECEIPT_API_PUB.CREATE_MISC (
                        ...
                        p_receipt_date         => TRUNC (SYSDATE), --TRUNC (LD_RECEIPT_DATE), -- Receipt date from table
                        p_gl_date              => TRUNC (SYSDATE), --TRUNC (LD_RECEIPT_DATE), -- GL date = Receipt date from table
			...
			L_CASH_RECEIPT_ID:=NULL;
			IF LC_FLAG_H <> 'E' AND NVL (LENGTH (TRIM (REPLACE(LC_COMMENTS_H,',',' '))), 0) <> 0
                THEN
                    FND_MSG_PUB.INITIALIZE;
                    FND_FILE.PUT_LINE(FND_FILE.LOG,'INSIDE API FOR: ' || 'C' || I.CONS_RCPT_NO);
					...
					AR_RECEIPT_API_PUB.CREATE_MISC (
                        ...
                        p_receipt_date         => TRUNC (SYSDATE), --TRUNC (LD_RECEIPT_DATE), -- WHAT SHOULD BE THE RECEIPT DATE
                        p_gl_date              => TRUNC (SYSDATE), --TRUNC (LD_RECEIPT_DATE), -- WHAT SHOULD BE THE GL_DATE
                    ...
                    THEN
						I.PROCESS_FLAG := 'S';
						FND_FILE.PUT_LINE(FND_FILE.LOG,'COMBINED RECEIPT CREATED...');
                        UPDATE XXAU_AR_TRANS_ERP_RCPT_T XATERT -- ADDB.ADDB_XXA_AR_TRANS_ERP_RCPT@ebstohr --XXAU_AR_TRANS_ERP_RCPT_T
  ...
  * Parameters :
	 ...
	 * AKALA 			1.3			30-MAR-2026 	Added P_UNIT parameter and updated by, process_date, updation_date for Update
     ***************************************************************************************************/

    PROCEDURE VALIDATION_REQUIRED(P_UNIT IN NUMBER)
    AS
        CURSOR CUR_REC IS
            SELECT COMPOSITE_RECEIPT_ID,
                   SERIAL_NUMBER,
                   ...
                   PROV_RECPTDT,
                   ERP_CUSTOMER_NUMBER
              FROM XXAU_AR_TRANS_ERP_RCPT_T A -- ADDB.ADDB_XXA_AR_TRANS_ERP_RCPT@ebstohr A --XXAU_AR_TRANS_ERP_RCPT_T
             WHERE     1 = 1
                   AND UPPER(ATTRIBUTE7) NOT LIKE '%CASH%' -- Added by AKALA on 12-MAR-2026 to remove the Cash Banks
                   AND PROCESS_FLAG IS NULL
                   AND CONS_RCPT_NO IS NOT NULL         ----- FOR CHECK ONLY ENABLED ON 02-04-2026 MADE BY ANIRUDH
				   AND EXISTS (SELECT 1 FROM xxau_accounting_unit_new@ebstohr
								   WHERE ORGANIZATION_CODE NOT IN ('HSO')
									 AND ORGANIZATION_CODE =SUBSTR(A.ATTRIBUTE11,1,3)
									 --AND FLEX_VALUE = P_UNIT 
									 AND ORGANIZATION_ID = P_UNIT 
							)
                   ;
    ...
    BEGIN
        ...
        LOOP
			...
			J.PROCESS_FLAG := 'P';
            LC_ERROR := NULL;
			J.PROCESS_DATE:= SYSDATE;
			--J.CREATION_DATE:= SYSDATE;
			--J.CREATED_BY:= FND_PROFILE.VALUE('USER_ID');
			J.LAST_UPDATE_DATE:= SYSDATE;
			J.LAST_UPDATED_BY:= FND_PROFILE.VALUE('USER_ID');
    ...
    */
                ...
                UPDATE XXAU_AR_TRANS_ERP_RCPT_T XATERT -- ADDB.ADDB_XXA_AR_TRANS_ERP_RCPT@ebstohr --XXAU_AR_TRANS_ERP_RCPT_T
				   ...
				   , PROCESS_DATE = J.PROCESS_DATE
				   --, CREATION_DATE = J.CREATION_DATE 
				   --, CREATED_BY = J.CREATED_BY
				   , LAST_UPDATE_DATE = J.LAST_UPDATE_DATE
				   , LAST_UPDATED_BY = J.LAST_UPDATED_BY
                 WHERE     1 = 1
     ...
     * ===============  =========   =============   ====================================================
     * AKALA            1.0         12-FEB-2026      Initial Version
     * AKALA            1.2         30-MAR-2026      Added P_UNIT parameter
     ***************************************************************************************************/

    PROCEDURE MAIN (P_ERRBUF OUT VARCHAR2, P_RETCODE OUT VARCHAR2, P_UNIT NUMBER)
    AS
    BEGIN
        FND_FILE.PUT_LINE(FND_FILE.LOG,'Calling VALIDATION_REQUIRED');

		VALIDATION_REQUIRED(P_UNIT);     -- should be enabled; disble only for testing

        FND_FILE.PUT_LINE(FND_FILE.LOG,
            '------------------------------------------------');
        FND_FILE.PUT_LINE(FND_FILE.LOG,'Calling ENTER_RECORDS');

	   ENTER_RECORDS(P_UNIT);
