...
IS
           ...
           SET PROCESS_FLAG =
				...
				, PROCESS_MESSAGE =
                   ...
                   || (SELECT ERROR_MESSAGE
                         FROM XXAU_AR_TRNSP_RCPT_ERR
                        WHERE ROWNUM < 2 AND (COMPOSITE_RECEIPT_ID = A.COMPOSITE_RECEIPT_ID OR COMPOSITE_RECEIPT_ID IS NULL) AND CONC_RCPT_ID = A.CONS_RCPT_NO)
				, PROCESS_DATE = SYSDATE 				--Added by AKALA on 30-Mar-2026
				, LAST_UPDATE_DATE = SYSDATE 			--Added by AKALA on 30-Mar-2026
				, LAST_UPDATED_BY = LN_LAST_UPDATED_BY 	--Added by AKALA on 30-Mar-2026
         WHERE     1 = 1
               AND COMPOSITE_RECEIPT_ID IS NOT NULL
               AND CONS_RCPT_NO IS NOT NULL
  ...
  * Parameters :
	 ...
	 * AKALA			1.3			30-MAR-2026		Added P_UNIT parameter and Changed Process date, P_Receipt_Date and P_gl_Date to Sysdate for all APIs
     ***************************************************************************************************/
             ...
             WHERE     1 = 1
				   ...
				   AND EXISTS (SELECT 1 FROM xxau_accounting_unit_new@ebstohr
								   WHERE ORGANIZATION_CODE NOT IN ('HSO')
									 AND ORGANIZATION_CODE =SUBSTR(XATERT.ATTRIBUTE11,1,3)
									 --AND FLEX_VALUE = P_UNIT 
									 AND ORGANIZATION_ID = P_UNIT  --Added by AKALA on 30-Mar-2026
									 )
				   ;
            ...
            THEN
                ...
                THEN
					...
					AR_RECEIPT_API_PUB.CREATE_MISC (
                        ...
                        p_receipt_date         => TRUNC (SYSDATE), --Added by AKALA on 30-Mar-2026 --TRUNC (LD_RECEIPT_DATE), -- Receipt date from table
                        p_gl_date              => TRUNC (SYSDATE), --Added by AKALA on 30-Mar-2026 --TRUNC (LD_RECEIPT_DATE), -- GL date = Receipt date from table
			...
			IF LC_FLAG_H <> 'E' AND NVL (LENGTH (TRIM (LC_COMMENTS_H)), 0) <> 0
                THEN
					...
					AR_RECEIPT_API_PUB.CREATE_MISC (
                        ...
                        p_receipt_date         => TRUNC (SYSDATE), --Added by AKALA on 30-Mar-2026 --TRUNC (LD_RECEIPT_DATE), -- WHAT SHOULD BE THE RECEIPT DATE
                        p_gl_date              => TRUNC (SYSDATE), --Added by AKALA on 30-Mar-2026 --TRUNC (LD_RECEIPT_DATE), -- WHAT SHOULD BE THE GL_DATE
  ...
  * Parameters :
    ...
    AS
        CURSOR CUR_REC IS
            SELECT COMPOSITE_RECEIPT_ID,
                   ...
                   PROV_RECPTDT,
                   ERP_CUSTOMER_NUMBER
              FROM ADDB.ADDB_XXA_AR_TRANS_ERP_RCPT@ebstohr A --XXAU_AR_TRANS_ERP_RCPT_T
             WHERE     1 = 1
    ...
    BEGIN
        ...
        LOOP
			...
			J.PROCESS_FLAG := 'P';
            LC_ERROR := NULL;
			J.PROCESS_DATE:= SYSDATE; --Added by AKALA on 30-Mar-2026
			--J.CREATION_DATE:= SYSDATE; --Added by AKALA on 30-Mar-2026
			--J.CREATED_BY:= FND_PROFILE.VALUE('USER_ID');
			J.LAST_UPDATE_DATE:= SYSDATE; --Added by AKALA on 30-Mar-2026
			J.LAST_UPDATED_BY:= FND_PROFILE.VALUE('USER_ID'); --Added by AKALA on 30-Mar-2026
